...
import { UserPlus, Mail, Lock, User, Landmark, ShieldCheck } from 'lucide-react';
...
export const RegisterAccount: React.FC = () => {
  ...
  const signupMutation = useMutation({
    mutationFn: () => authService.signup({ name, email, password, role }),
    onSuccess: (res) => {
      if (res.success) {
  ...
  return (
    <div className="max-w-md mx-auto px-4 pb-24 pt-32 space-y-8 relative">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl -z-10 animate-pulse"></div>
      ...
      <div className="glass-panel border border-white/5 p-8 rounded-3xl shadow-2xl space-y-6 bg-slate-900/30">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-text-secondary text-xs sm:text-sm">
            Join the EventFlow network to find and host gatherings.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 p-3.5 rounded-xl border text-xs font-semibold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                  role === 'User'
                    ? 'bg-indigo-500/10 border-indigo-500 text-white'
                    : 'bg-slate-950 border-white/5 text-text-secondary hover:text-white'
                }`}
              >
          ...
          >
            ...
            {signupMutation.isPending ? 'Registering Account...' : 'Sign Up'}
          </button>
        </form>
        ...
        <div className="space-y-3 pt-2">
          <div className="relative flex py-1 items-center">
            ...
            <span className="flex-shrink mx-4 text-text-muted text-[10px] uppercase font-bold tracking-wider">Or Register with Social</span>
            <div className="flex-grow border-t border-white/5"></div>
          </div>
        ...
        <p className="text-[10px] text-text-muted leading-relaxed text-center flex items-center justify-center gap-1">
          <ShieldCheck className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
          Securely stored locally using demo persistence methods.
        </p>

        {/* Sign in link */}
        <p className="text-center text-xs text-text-secondary border-t border-white/5 pt-4">
